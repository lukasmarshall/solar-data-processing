...
def printTimeseries():
	...
	for strikePrice in strikePrices:
		...
		averagesPath = "/Users/lukemarshall/Documents/Workspace/Thesis/simulationResults/market/shapedCapMonthlyPrice/"+str(strikePrice)+".csv"
		
		averagesFile = open(averagesPath, 'w')
		averagesFile.write("Location, Monthly Price Fixed, Monthly Price Tracking\n")
		...
		for location in locations.getLocations():
				...
				for i in np.arange(data.shape[0]):
					...
					if hour >= contractStartHour and hour <= contractEndHour: 
						...
						if price > strikePrice:
							...
							if output > (capFactor * timePeriodHrs * mwNameplate):
								countFixedDemandMet = countFixedDemandMet + 1
							if trackingOutput > (capFactorTracking * timePeriodHrs * mwNameplate):
			...
			numYears = float(7)
			numMonths = float(12)
			totalMonths = numYears * numMonths
			fixedPrice = fixedPrice / totalMonths
			trackingPrice = trackingPrice / totalMonths

			averageString = state+", "+str(fixedPrice)+","+str(trackingPrice)+"\n"
