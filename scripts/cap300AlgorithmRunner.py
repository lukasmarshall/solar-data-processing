from model.algorithms.cap300Algorithm import Cap300Algorithm as Algorithm
...
def printTimeseries(tracking, storageMWh, fractionContracted = 0.25, shaved = False):
	if shaved:
		shavedString = "Shaved"
	else:
		shavedString = ""
	...
	for spec in batteryStorage.getBatterySpecsList():
		...
		stateAveragesFile = open(averagesPath, 'w')
		...
		for location in locations.getLocations():
			...
			state = location[2]
			capValuePerMWh = location[5]
			print "Cap Value: " +str(capValuePerMWh)
			startYear = 2005
			endYear = 2011
			averageRevenue = 0
			...
			for year in np.arange(startYear, endYear + 1):
				for month in np.arange(1,13):
					...
					a = Algorithm(strikePrice = strikePrice, fractionContracted = fractionContracted, numStates = 30, batteryType = batteryType, capped = False, shaved = shaved, capValuePerMWh = capValuePerMWh)
